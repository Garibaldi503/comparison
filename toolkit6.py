...
with tab1:
    ...
    <div class='wrap'>
      <table>
        <thead>
          <tr>
            <th>Area</th>
            <th>Typical ERP Output</th>
            <th>AI/ML-Driven Insight</th>
          </tr>
        </thead>
        <tbody>
          {table_rows_html}
    ...
    <style>
      ...
      thead th {{ background:var(--navy); color:#fff; padding:12px; font-weight:700; text-align:left; }}
      ...
      .brand {{ text-align:right; color:#333; padding:8px 6px; }}
    </style>
    """
...
"""
    ...
    b, a = np.polyfit(ln_p, ln_q, deg=1)
    elasticity = float(b)
    ...
    ax.plot(p_grid, [q_hat(p) for p in p_grid], label="Fitted", color=NAVY)
    ax.axvline(base_p, ls="--", color=NAVY)
    ax.axvline(new_p, ls="--", color=GOLD)
    ax.set_xlabel("Price")
